...
const UploadInput = ({ className, targetRef, placeholder, emoticon }: UploadInputProps) => {
  ...
  const changeHandler = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (inputRef.current && files) inputRef.current.placeholder = files[0].name;
  };
  ...
  return (
    <label className={cn("relative flex justify-between w-full cursor-pointer", className)}>
      ...
      />
      <span
        className={cn("w-[43px] h-[34px] bg-white flex items-center justify-center rounded-lg shadow-[0px_2px_6px_rgba(0,0,0,0.18)]", {
          ["hidden"]: !emoticon,
        })}
      >
        {emoticon && emoticon}
