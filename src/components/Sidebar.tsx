...
const Sidebar = ({ open }: SidebarProps) => {
  ...
  return (
    <div
      className={cn(
        "absolute right-0 top-[50px] w-4/5 max-w-[350px] h-[calc(100vh-102px)] overflow-auto pt-5 px-2  shadow-2xl border border-white border-solid backdrop-blur-sm bg-[#F0F0F099] rounded-s-3xl transition duration-500 ease-in-out opacity-100",
        {
      ...
      <div className="mt-5 flex justify-center ">
        <div className="flex flex-col w-11/12 text-xs text-slate-600">
          ...
          <button onClick={setToggle} className="block w-full text-left">
            Business Information <span className={cn("inline-block duration-300 transform rotate-0", { ["rotate-180 "]: toggle })}>▼</span>
          </button>
